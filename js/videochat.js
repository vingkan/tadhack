function initVideoChat(userToCall){

  var params = getHashParams(),
      mode = typeof(params.mode)=='undefined'?'webrtc':params.mode,
      username = window.assignedUserName,
      password = window.assignedUserName + 'pass',
      application_name = 'tadhack',//typeof(params.appname)=='undefined'?'videochat':params.appname,
      account_name = 'vinesh',//params.accname,
      dialog,
      showLog = true,
      currentCall = null,
      outboundCall = null;
  ...
  function createCall(cid) {
    var callerID = cid || document.getElementById('phonenum').value;
    $('#callButton').replaceWith('<button type="button" class="btn btn-danger" id="cancelButton">Cancel</button>');
    $('#callButton').remove();
    $('#cancelButton').click(function() {
      currentCall.hangup();
    });
    log("Calling to "+callerID);
    outboundCall = currentCall = voxAPI.call(callerID, true, "TEST CUSTOM DATA", {"X-DirectCall": "true"});
    currentCall.addEventListener(VoxImplant.CallEvents.Connected, onCallConnected);
    currentCall.addEventListener(VoxImplant.CallEvents.Disconnected, onCallDisconnected);
    currentCall.addEventListener(VoxImplant.CallEvents.Failed, onCallFailed);
  }
  ...
  function disconnectCall() {
    if (currentCall != null) {        
      log("Disconnect");
      currentCall.hangup();
      window.disconnectCallFromFirebase();
    }
  } 
